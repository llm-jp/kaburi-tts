...
使い方:
  ...
  # (内部データ保有者向け) コーパス manifest の template chunk を参照に使う場合
  python scripts/kaburi_cli.py dialog.txt --manifest <train.jsonl> \
      --speakerA <SPK_A> --speakerB <SPK_B> --template_chunk <chunk_id> \
      --utt-timing-jsonl <utt_timing.jsonl>
...
- 左チャネル=話者A、 右チャネル=話者B。
- --mode pred は学習済み duration predictor、 --mode stat は学習不要の統計配置でタイミングを決める。
...
def main() -> None:
    ...
    ap.add_argument("--mode", choices=["pred", "stat"], default="pred",
                    help="pred=duration predictor / stat=統計配置（既定 pred）")
    ...
    ap.add_argument("--predictor-ckpt", default=None, help="local ckpt; omit to download from HF")
    ap.add_argument("--gap-bias-frames", type=float, default=0.0,
                    help="predictor の gap への一律補正 (frame, 25fps)。既定 0 = 無補正")
