...
def _resolve_dialog_seed(base_seed: int, offset: int, dialog_id: str, mode: str) -> int:
    if mode == "order":
        return int(base_seed + offset)
    match = re.fullmatch(r"c(\d+)", dialog_id)
    if match is None:
        raise ValueError(
            f"--seed_mode=dialog-id requires IDs like c001, got {dialog_id!r}"
        )
    return int(base_seed + int(match.group(1)) - 1)
...
def main() -> None:
    ...
    ap.add_argument("--force", action="store_true", help="既存WAVがあっても再合成する")
    ...
    ap.add_argument(
        "--seed_mode",
        choices=["order", "dialog-id"],
        default="order",
        help="order: 従来の入力順offset、dialog-id: cNNNから安定seedを作る（部分再合成用）",
    )
    # kaburi_stat
    ...
    ap.add_argument("--overflow_policy", choices=["error", "gap-only", "global"], default="gap-only",
                    help="error: fitが必要なら不合格、gap-only: 正gapを先に縮小、"
                         "global: 旧式の全体圧縮。eval preflightではerrorを使用")
    ...
    ap.add_argument("--duration_override_json", default=None,
                    help="診断用: dialog_id -> 発話ごとのPHONE duration配列を持つJSON。"
                         "predictor durationを置換し、phone sourceとの直交比較に使う")
    # irodori_stat
...
def _run_kaburi(args, dialogs, speakers, device, out_dir):
    ...
    if args.mode == "kaburi_pred":
        ...
        if args.duration_override_json:
            duration_overrides = json.loads(Path(args.duration_override_json).read_text(encoding="utf-8"))
            duration_overrides = duration_overrides.get("dialogs", duration_overrides)
            print(f"[duration_override] loaded {args.duration_override_json}", flush=True)
    ...
    n_ok = n_miss = n_overflow = 0
    for off, d in enumerate(dialogs):
        did = d["id"]
        dialog_seed = _resolve_dialog_seed(args.seed, off, did, args.seed_mode)
        out_wav = out_dir / f"{did}.wav"
        if out_wav.exists() and not args.fit_only and not args.force:
            n_ok += 1; continue
        sp = speakers.get(did)
        if sp is None:
        ...
        try:
            if args.mode == "kaburi_stat":
                placements, fit_meta = SPB.make_statistical_placements(
                    ...
                    overflow_policy=args.overflow_policy, return_fit_info=True)
                ...
                timing_meta = {**fit_meta, **timing_meta}
                ...
                pA, pB, aA, aB, timing_meta = SPRED._predict_raster_from_item(
                    ...
                    dur_sample_temp=args.dur_sample_temp, dur_seed=dialog_seed,
                    ...
                    duration_override_by_utt=(
                        duration_overrides.get(did) if duration_overrides is not None else None
                    ))
                batch = SPRED._replace_batch_raster(dict(base_batch), pA, pB, aA, aB, boundary_radius)
        except (SPB.PlacementOverflowError, SPRED.RasterOverflowError) as exc:
            n_overflow += 1
            print(f"[overflow] {did}: {exc.reason} {exc.info}", flush=True)
            if meta_f is not None:
                meta_f.write(json.dumps({
                    "dialog_id": did,
                    "mode": args.mode,
                    "status": "overflow",
                    "overflow_reason": exc.reason,
                    "speaker_A": cA,
                    "speaker_B": cB,
                    "template_chunk": template_chunk,
                    "n_utts": int(len(d["utts"])),
                    "n_phones": int(sum(int(u.get("n_phones", len(u.get("phone_ids", []))))
                                        for u in d["utts"])),
                    **exc.info,
                }, ensure_ascii=False) + "\n")
                meta_f.flush()
        ...
        wav = synth_acoustic(acoustic, codec, batch, device=device, num_steps=args.num_steps,
                        seed=dialog_seed, cfg_scale=args.cfg_scale, pct8=pct8)
    ...
    print(f"[{args.mode}] done ok={n_ok} miss={n_miss} overflow={n_overflow} -> {out_dir}", flush=True)
    if meta_f is not None:
        meta_f.close()
    if args.fit_only and (n_miss > 0 or n_overflow > 0):
        raise SystemExit(2)
