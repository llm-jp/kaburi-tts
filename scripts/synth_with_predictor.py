...
class RasterOverflowError(ValueError):
    """Raised when an evaluation raster would require timing compression."""

    def __init__(self, reason: str, info: dict):
        self.reason = str(reason)
        self.info = dict(info)
        super().__init__(f"{self.reason}: {json.dumps(self.info, ensure_ascii=False, sort_keys=True)}")
...
def _fit_durations_to_T(
    durations, token_type, channel_id, attn_mask, T: int, *,
    overflow_policy: str = "gap-only",
):
    """Audit decoded token totals without modifying any token duration.

    The historical implementation reduced PRE/FIRST_SIL from a per-channel
    token sum before reconstructing the two-speaker timeline.  That sum is not
    the rendered end time (gap_blend and cross-channel ordering still apply),
    so valid dialogs were silently retimed.  Overflow is now decided only from
    the reconstructed timeline in _fit_timeline_by_gap_scale.
    """
    if overflow_policy not in {"error", "gap-only", "global"}:
        raise ValueError(f"unknown overflow_policy: {overflow_policy}")
    fitted = durations.clone()
    info = {"duration_fit_policy": overflow_policy}
    for ch in [0, 1]:
        total_before = _channel_total_len(fitted, token_type, channel_id, attn_mask, ch)
        overflow_before = max(0.0, total_before - T)
        info[f"ch{ch}_duration_total_before"] = float(total_before)
        info[f"ch{ch}_overflow_before"] = float(overflow_before)
        info[f"ch{ch}_sil_reduced"] = 0.0
        info[f"ch{ch}_clamped"] = 0.0
    return fitted, info


def _apply_phone_duration_override(
    durations, token_type, utt_index, attn_mask,
    duration_override_by_utt: list[list[float]],
    dialog_rank=None,
):
    fitted = durations.clone()
    phone_mask = (token_type == TOKEN_TYPE_PHONE) & attn_mask.bool()
    # utt_index is channel-local, so A:0 and B:0 are different utterances.
    # dialog_rank is the stable global utterance index used by override JSON.
    grouping = dialog_rank if dialog_rank is not None else utt_index
    seen_utts = set()
    for k in range(token_type.shape[0]):
        if bool(phone_mask[k]):
            seen_utts.add(int(grouping[k].item()))
    if len(duration_override_by_utt) != len(seen_utts):
        raise ValueError(
            f"duration override utterance count mismatch: "
            f"{len(duration_override_by_utt)} != {len(seen_utts)}"
        )
    for u in sorted(seen_utts):
        values = duration_override_by_utt[u]
        indices = [
            k for k in range(token_type.shape[0])
            if bool(phone_mask[k]) and int(grouping[k].item()) == u
        ]
        if len(values) != len(indices):
            raise ValueError(
                f"duration override phone count mismatch for utt {u}: "
                f"{len(values)} != {len(indices)}"
            )
        for k, value in zip(indices, values):
            fitted[k] = max(1.0, float(value))
    return fitted
...
def _timeline_max_phone_end(timeline, token_type, attn_mask) -> float:
    """Return the final PHONE end frame, ignoring SIL/padding tokens."""
    ends = timeline["token_ends"]
    token_type = token_type.cpu()
    attn_mask = attn_mask.cpu()
    phone_mask = (token_type == TOKEN_TYPE_PHONE) & attn_mask.bool()
    if not bool(phone_mask.any()):
        return 0.0
    return float(ends[phone_mask].max().item())


def _fit_timeline_by_gap_scale(
    durations, gaps, token_type, channel_id, utt_index, mask, order, T: int, *,
    gap_blend: float, fit_margin: int = 5, n_iter: int = 16,
    overflow_policy: str = "gap-only",
):
    """Fit a predicted timeline by reducing positive inter-utterance waits first.

    PHONE durations are preserved. Global PHONE compression is left to the
    rasterizer only when even gap_scale=0 cannot fit the PHONE spans in T.
    """
    if overflow_policy not in {"error", "gap-only", "global"}:
        raise ValueError(f"unknown overflow_policy: {overflow_policy}")
    target = float(max(1, T - fit_margin))
    timeline = _reconstruct_with_any_gap(
        durations, gaps, token_type, channel_id, utt_index, mask, order, T,
        gap_blend=gap_blend, gap_scale=1.0,
    )
    raw_max_end = _timeline_max_phone_end(timeline, token_type, mask)
    base_info = {
        "raw_max_end": float(raw_max_end),
        "fit_target": float(target),
        "overflow_policy": overflow_policy,
    }
    if raw_max_end <= target:
        return timeline, {**base_info, "gap_scale": 1.0,
                          "gap_fit_used": False, "gap_fit_success": True,
                          "gap_fit_max_end": float(raw_max_end)}
    if overflow_policy == "error":
        raise RasterOverflowError("timeline_overflow", base_info)
    if overflow_policy == "global":
        return timeline, {**base_info, "gap_scale": 1.0,
                          "gap_fit_used": False, "gap_fit_success": False,
                          "gap_fit_max_end": float(raw_max_end)}
    timeline0 = _reconstruct_with_any_gap(
        durations, gaps, token_type, channel_id, utt_index, mask, order, T,
        gap_blend=gap_blend, gap_scale=0.0,
    )
    max0 = _timeline_max_phone_end(timeline0, token_type, mask)
    if max0 > target:
        return timeline0, {**base_info, "gap_scale": 0.0,
                           "gap_fit_used": True, "gap_fit_success": False,
                           "gap_fit_max_end": float(max0)}
    lo, hi = 0.0, 1.0
    best_timeline, best_max_end = timeline0, max0
    for _ in range(n_iter):
        mid = (lo + hi) * 0.5
        cand = _reconstruct_with_any_gap(
            durations, gaps, token_type, channel_id, utt_index, mask, order, T,
            gap_blend=gap_blend, gap_scale=mid,
        )
        cand_max = _timeline_max_phone_end(cand, token_type, mask)
        if cand_max <= target:
            lo = mid; best_timeline, best_max_end = cand, cand_max
        else:
            hi = mid
    return best_timeline, {**base_info, "gap_scale": float(lo),
                           "gap_fit_used": True, "gap_fit_success": True,
                           "gap_fit_max_end": float(best_max_end)}
...
def _predict_raster_from_item(model, pred_item: dict, device: torch.device, *, T: int, gap_blend: float,
                              ...
                              overflow_policy: str = "gap-only",
                              duration_override_by_utt: list[list[float]] | None = None):
    ...
    duration_source = "predictor"
    if duration_override_by_utt is not None:
        duration_source = "override"
        durations = _apply_phone_duration_override(
            durations, token_type, utt_index, mask, duration_override_by_utt,
            dialog_rank=bd["timing_dialog_rank"][0],
        )
    durations, fit_info = _fit_durations_to_T(
        durations, token_type, channel_id, mask, T,
        overflow_policy=overflow_policy,
    )
    ...
    # --- gap-only fit: 正の発話間 gap を先に縮め、phone 長は保持する。gap_scale=0 でも T に収まらない
    #     場合のみ _timeline_to_raster が last-resort で全体 scale する（= 全体一律圧縮によるカタコト化バグ
    #     の修正 2026-07-30。旧: timeline>T で phone も一律圧縮していた）。---
    FIT_MARGIN = 5
    tt_cpu, ph_cpu, ch_cpu, am_cpu = token_type.cpu(), phone_id.cpu(), channel_id.cpu(), mask.cpu()
    timeline, gap_fit_info = _fit_timeline_by_gap_scale(
        durations, gaps, token_type, channel_id, utt_index, mask, order, T,
        gap_blend=gap_blend, fit_margin=FIT_MARGIN,
        overflow_policy=overflow_policy,
    )
    raw_max_end = gap_fit_info["raw_max_end"]
    fitted_max_end = _timeline_max_phone_end(timeline, token_type, mask)
    fit_target = float(max(1, T - FIT_MARGIN))
    global_scale = min(1.0, fit_target / max(fitted_max_end, 1.0))
    print(f"[fit] raw_max_end={raw_max_end:.0f} target={T-FIT_MARGIN} "
          f"gap_scale={gap_fit_info['gap_scale']:.3f} used={gap_fit_info['gap_fit_used']} "
          f"success={gap_fit_info['gap_fit_success']} global_scale={global_scale:.3f}", flush=True)
    ...
    phone_mask = (token_type == TOKEN_TYPE_PHONE) & mask.bool()
    phone_durations = durations[phone_mask]
    timeline_spans = (
        timeline["token_ends"][phone_mask.cpu()] - timeline["token_starts"][phone_mask.cpu()]
        if bool(phone_mask.any()) else torch.empty(0)
    )
    meta = {
        **fit_info,
        **gap_fit_info,
        "raw_max_end": float(raw_max_end),
        "fitted_max_end": float(fitted_max_end),
        "global_scale": float(global_scale),
        "global_scale_used": bool(global_scale < 1.0),
        "pred_phone_duration_min": float(phone_durations.min().item()) if phone_durations.numel() else 0.0,
        "timeline_phone_span_min": float(timeline_spans.min().item()) if timeline_spans.numel() else 0.0,
        "n_timeline_phone_spans_lt_1": int((timeline_spans < 1.0).sum().item()),
...
def main():
    ...
    if not run_chunk_ids:
        raise ValueError("--chunk-ids を指定してください")
