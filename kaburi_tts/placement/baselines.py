...
class PlacementOverflowError(ValueError):
    """Raised when an evaluation placement would require timing compression."""
    ...
    *,
    ...
    for i, u in enumerate(utts):
        ...
        if i == 0:
            ...
            if gap > 0:  # 正 gap(pause)のみ gap_scale で縮小、overlap(負 gap)は保持
                gap = int(round(gap * gap_scale))
...
def make_statistical_placements(
    ...
    *,
    ...
    末尾超過時は **phone duration を保持したまま正 gap のみ** binary search で縮める
    (旧実装は start/dur を一律 scale し phone を圧縮→カタコト化していた)。
    正 gap をゼロまで削っても speech region だけで T を超える場合に限り、
    最終手段として _scale_placements_to_fit(duration scale) にフォールバックする。
    """
    if overflow_policy not in {"error", "gap-only", "global"}:
        raise ValueError(f"unknown overflow_policy: {overflow_policy}")

    def _build(gap_scale: float) -> list[dict]:
        return _stat_placements_build(
            utts, T, seed=seed, start_offset=start_offset, end_margin=end_margin,
            overlap_prob=overlap_prob, duration_scale=duration_scale, stats=stats,
            gap_scale=gap_scale,
        )
    def _max_end(ps: list[dict]) -> int:
        return max((int(p["start"]) + int(p["dur"]) for p in ps), default=0)
    def _finish(ps: list[dict], info: dict):
        return (ps, info) if return_fit_info else ps
    def _duration_ratio(before: list[dict], after: list[dict]) -> float:
        den = float(sum(int(p["dur"]) for p in before))
        return float(sum(int(p["dur"]) for p in after)) / den if den > 0 else 1.0

    target_end = max(1, T - end_margin)
    full = _build(1.0)
    raw_max_end = _max_end(full)
    base_info = {
        "raw_max_end": int(raw_max_end),
        "fit_target": int(target_end),
        "overflow_policy": overflow_policy,
    }
    if raw_max_end <= target_end:
        return _finish(full, {
            **base_info,
            "gap_scale": 1.0,
            "gap_fit_used": False,
            "gap_fit_success": True,
            "fitted_max_end": int(raw_max_end),
            "global_scale": 1.0,
            "global_scale_used": False,
        })
    if overflow_policy == "error":
        raise PlacementOverflowError("timeline_overflow", base_info)
    if overflow_policy == "global":
        scaled = _scale_placements_to_fit(full, T, end_margin)
        scale = _duration_ratio(full, scaled)
        return _finish(scaled, {
            **base_info,
            "gap_scale": 1.0,
            "gap_fit_used": False,
            "gap_fit_success": False,
            "fitted_max_end": int(_max_end(scaled)),
            "global_scale": float(scale),
            "global_scale_used": bool(scale < 1.0),
        })
    zero = _build(0.0)
    if _max_end(zero) > target_end:
        # gap をゼロまで削っても超過 = speech alone > T。最終手段の duration scale。
        scaled = _scale_placements_to_fit(zero, T, end_margin)
        scale = _duration_ratio(zero, scaled)
        return _finish(scaled, {
            **base_info,
            "gap_scale": 0.0,
            "gap_fit_used": True,
            "gap_fit_success": False,
            "gap_fit_max_end": int(_max_end(zero)),
            "fitted_max_end": int(_max_end(scaled)),
            "global_scale": float(scale),
            "global_scale_used": bool(scale < 1.0),
        })
    # 収まる最大の gap_scale を binary search(phone は不変、gap のみ縮む)。
    lo, hi, best = 0.0, 1.0, zero
    for _ in range(16):
        mid = (lo + hi) / 2.0
        cand = _build(mid)
        if _max_end(cand) <= target_end:
            lo, best = mid, cand
        else:
            hi = mid
    return _finish(best, {
        **base_info,
        ...
        "gap_fit_used": True,
        "gap_fit_success": True,
        "gap_fit_max_end": int(_max_end(best)),
        "fitted_max_end": int(_max_end(best)),
        "global_scale": 1.0,
        "global_scale_used": False,
    })
...
def placements_to_raster(placements: list[dict], T: int, phone_vocab_size: int,
    ...
    meta = {
        ...
        "placement_duration_min": min((int(p["dur"]) for p in placements), default=0),
        "placement_frames_per_phone_min": min(
            (float(p["dur"]) / max(1, len(p["phone_ids"])) for p in placements),
            default=0.0,
        ),
        "placements": [
            {
...
def main() -> None:
    ...
    ap.add_argument("--overflow-policy", choices=["error", "gap-only", "global"], default="gap-only",
                    help="error: fitが必要なら失敗、gap-only: 正gapを先に縮小、global: 旧式の全体圧縮")
    args = ap.parse_args()
