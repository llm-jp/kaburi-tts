...
class Normal2StreamDataset(Dataset):
    ...
    def _load_ref(self, ref_path: str) -> torch.Tensor:
        ...
        if cached is None:
            d = torch.load(ref_path, map_location="cpu", weights_only=False)
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        c = torch.load(e["chunk_path"], map_location="cpu", weights_only=False)
        T = self.latent_T
...
class Event2StreamDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict:
        e = self.entries[self._index_map[idx]]
        ev = torch.load(e["event_path"], map_location="cpu", weights_only=False)
